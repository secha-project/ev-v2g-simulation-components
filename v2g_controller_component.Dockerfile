...
LABEL org.opencontainers.image.description "Docker image for the v2g controller component for the SimCES platform."

# create the required directories inside the Docker image
RUN mkdir -p /v2g_controller_component
RUN mkdir -p /init
RUN mkdir -p /logs
...
COPY v2g_controller_component/ /v2g_controller_component/
...
CMD [ "python3", "-u", "-m", "v2g_controller_component.v2g_controller_component" ]